...
function Pay(){
  ...
  useEffect(()=>{supabase.rpc('worker_hourly_summary',{p_from:monthStart(),p_to:day()}).then(({data})=>setX(data?.[0]))},[])
  return <section><h2>Почасовая оплата</h2><div className="cards"><div className="card"><span className="muted">Начислено по часам</span><div className="kpi">{money(x?.hourly_pay)}</div></div><div className="card"><span className="muted">Учтено рабочего времени</span><div className="kpi">{hours(x?.worked_minutes)}</div></div></div><div className="panel">Рабочему показывается только почасовая часть начисления.</div></section>
}
