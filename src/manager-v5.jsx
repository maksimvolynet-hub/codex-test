...
import {HRPanel} from './manager-hr-v6'
...
export function Manager({profile}){
  ...
  async function loadRequests(){
    ...
    if(error){setLoadError(error.message||'Не удалось загрузить запросы повторной смены');return}
    const rows=data||[]
    if(!rows.length){setRequests([]);setLoadError('');return}
    const workerIds=[...new Set(rows.map(x=>x.worker_id).filter(Boolean))]
    const {data:people,error:peopleError}=await supabase.from('profiles').select('id,full_name').in('id',workerIds)
    if(peopleError){setLoadError(peopleError.message||'Не удалось загрузить имена рабочих');setRequests(rows.map(x=>({...x,worker_name:'Рабочий'})));return}
    const names=Object.fromEntries((people||[]).map(x=>[x.id,x.full_name]))
    setRequests(rows.map(x=>({...x,worker_name:names[x.worker_id]||'Рабочий'})));setLoadError('')
  }

  async function resolve(id,approve){const comment=prompt(approve?'Комментарий НЦ к повторной смене:':'Причина отказа:','')||'';setBusy(true);const {error}=await supabase.rpc('resolve_shift_reopen',{p_request_id:id,p_approve:approve,p_comment:comment});setBusy(false);if(error){alert(error.message);return}await loadRequests()}

  useEffect(()=>{
    loadRequests()
    const timer=setInterval(loadRequests,2000)
    const channel=supabase.channel(`manager-shift-reopen-${profile.id}`).on('postgres_changes',{event:'*',schema:'public',table:'shift_reopen_requests'},loadRequests).on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:`user_id=eq.${profile.id}`},loadRequests).subscribe()
