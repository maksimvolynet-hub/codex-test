...
function anniversaryYears(v){if(!v)return 0;const t=day(),y=Number(t.slice(0,4))-Number(String(v).slice(0,4));return Math.max(0,y)}
function Anniversary({profile}){if(profile?.role!=='worker')return null;const t=day(),md=t.slice(5,10),work=profile.employment_start_date&&String(profile.employment_start_date).slice(5,10)===md,official=profile.official_employment&&profile.official_start_date&&String(profile.official_start_date).slice(5,10)===md;if(!work&&!official)return null;return <div style={{margin:'10px 20px 0',padding:'12px 14px',borderRadius:'12px',background:'#fff7ed',border:'1px solid #fdba74'}}><b>🎉 Поздравляем, {profile.full_name}!</b>{work&&<div>Сегодня {anniversaryYears(profile.employment_start_date)}-я годовщина вашей работы в ФЕРРУМ.</div>}{official&&<div>Сегодня {anniversaryYears(profile.official_start_date)}-я годовщина официального трудоустройства.</div>}</div>}
export function Header({profile,title}){return <><header><div><div className="logo small">ФЕРРУМ</div><div className="muted">{title}</div></div><div className="row"><b>{profile.full_name}</b><button className="ghost" onClick={()=>supabase.auth.signOut()}>Выйти</button></div></header><Anniversary profile={profile}/></>}
