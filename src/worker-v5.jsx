...
function Today({profile,goReport}){
  ...
  async function load(){
    ...
    setReportSent(Boolean(attRes.data?.report_submitted_at));setAvailable(Boolean(avRes.data));setEarly(earlyRes.data?.[0]||null);setReopen(reopenRes.data?.[0]||null);setLoading(false)
  }
  ...
  return <section><div className="head"><div><h2>Сегодня</h2><div className="muted">{day()} · смена №{shift.sequence_no}</div></div><Badge tone="green">На рабочем месте</Badge></div>
    ...
    {active?<div className="task active"><div className="row between"><Badge tone={active.status==='active'?'green':active.close_approved_at?'blue':'orange'}>{active.close_approved_at?'НЦ разрешил закрыть':STATUS[active.status]}</Badge><Timer startedAt={active.started_at}/></div><h2>{active.title}</h2><div className="order">Заказ {active.order_number} · позиция {active.position_number}</div><p>{active.description}</p><div>План: <b>{active.planned_minutes} мин</b></div>{active.status==='active'&&<><div className="split"><Field label="Фото до (необязательно)"><input type="file" accept="image/*" capture="environment" onChange={e=>upload(active,e.target.files?.[0],'before')}/></Field><Field label="Фото после (обязательно при завершении)"><input type="file" accept="image/*" capture="environment" onChange={e=>upload(active,e.target.files?.[0],'after')}/></Field></div><div className="row"><button className="secondary" onClick={()=>stop(active.id)}>Не могу продолжить</button><button onClick={()=>done(active.id)}>Завершить задание</button></div></>}{active.status==='stop_requested'&&!active.close_approved_at&&<div className="warning">Запрос отправлен НЦ. Ожидайте решения.</div>}</div>:<div className="panel">Активного задания нет.</div>}

    <h2>Очередь</h2>
    {queue.length?queue.map((x,i)=><div className="task" key={x.id}><div className="row between"><b>#{i+1} · {x.title}</b>{x.carryover&&<Badge tone="orange">Продолжить</Badge>}</div><div>Заказ {x.order_number} · поз. {x.position_number}</div><div className="muted">План {x.planned_minutes} мин</div>{!active&&i===0&&<button onClick={()=>start(x.id)}>{x.carryover?'Продолжить задание':'Начать задание'}</button>}</div>):<Empty>Следующих заданий нет — подойдите к начальнику цеха</Empty>}

    <div className="panel end-shift"><h2>Завершение смены</h2>{canFinish?<button className="big-action" onClick={()=>active?setFinishAsk(true):goReport()}>Завершить смену и написать отчёт</button>:early?.status==='pending'?<div className="warning">Запрос на раннее завершение отправлен. Ожидайте решение НЦ.</div>:<button className="big-action" onClick={requestEarly}>Запросить раннее завершение смены</button>}</div>
...
function Pay(){const [x,setX]=useState(null);useEffect(()=>{supabase.rpc('worker_hourly_summary',{p_from:monthStart(),p_to:day()}).then(({data})=>setX(data?.[0]))},[]);return <section><h2>Почасовая оплата</h2><div className="cards"><div className="card"><span className="muted">Начислено по часам</span><div className="kpi">{money(x?.hourly_pay)}</div></div><div className="card"><span className="muted">Учтено рабочего времени</span><div className="kpi">{hours(x?.worked_minutes)}</div></div></div></section>}

function WorkerHelp(){return <section className="help"><h2>Инструкция рабочего</h2><div className="panel steps"><h3>Конец смены</h3><ol><li>Нажмите «Завершить смену и написать отчёт».</li><li>Если есть текущее задание, программа спросит, завершили ли вы его.</li><li>Если «Да» — задание закрывается обычным способом, фото после обязательно.</li><li>Если «Нет» — время останавливается, задание переносится первым на следующую смену.</li><li>На следующей смене нажмите «Продолжить задание».</li></ol></div></section>}
