...
import {Worker} from './worker-history-v7'
import './v2.css'
...
function Login(){
  ...
  async function checkConnection(){
    ...
    try{const r=await fetch(PROJECT_HEALTH,{signal:controller.signal,cache:'no-store'});clearTimeout(timer);if(r.ok)setMsg('Связь с сервером есть. Если вход не проходит — проверьте раскладку пароля и попробуйте в Chrome.');else setMsg(`Сервер доступен, но вернул ошибку ${r.status}.`)}catch(e){clearTimeout(timer);setMsg('Телефон не может связаться с сервером ФЕРРУМ (Supabase). Попробуйте Chrome и переключите мобильный интернет/Wi‑Fi. VPN не должен быть обязательным.')}
  }
  async function go(e){e.preventDefault();setBusy(true);setMsg('');let email=u.trim();if(!email.includes('@'))email=email.toLowerCase()+'@ferrum.local';try{const {error}=await supabase.auth.signInWithPassword({email,password:p});if(error){const text=String(error.message||'').toLowerCase();if(text.includes('fetch')||text.includes('network')||text.includes('failed'))setMsg('Нет связи с сервером авторизации. Логин и пароль могут быть правильными. Нажмите «Проверить связь».');else if(text.includes('invalid login credentials'))setMsg('Сервер ответил: неверный логин или пароль. Проверьте раскладку клавиатуры и регистр пароля.');else setMsg(`Ошибка входа: ${error.message}`)}}catch(e){setMsg('Телефон не смог подключиться к серверу авторизации. Нажмите «Проверить связь».')}finally{setBusy(false)}}
...
function App(){
  ...
  useEffect(()=>{if(!s?.user){setProfile(null);setBlocked(false);return}let live=true;async function check(){const {data}=await supabase.from('profiles').select('*').eq('id',s.user.id).single();if(!live)return;if(data&&!data.active){setBlocked(true);setProfile(null);await supabase.auth.signOut();return}setBlocked(false);setProfile(data||null)}check();const i=setInterval(check,5000);return()=>{live=false;clearInterval(i)}},[s])
